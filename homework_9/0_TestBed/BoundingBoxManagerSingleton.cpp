...
void BoundingBoxManagerSingleton::CalculateCollision(void)
{
	...
	for(int nBox = 0; nBox < m_nBoxs; nBox++)
	{
		...
		m_lBox[nBox]->n_Color = vector3(1.0f);
	}

	//Now the actual check
	for(int i = 0; i < m_nBoxs - 1; i++)
	{
		vector3 box1HalfSize = m_lBox[i]->n_Size / 2.0f;
		vector3 trans1 = vector3(m_lMatrix[i][3]);
		vector3 box1center = trans1 + m_lBox[i]->n_Center;

		for(int j = i + 1; j < m_nBoxs; j++)
		{
			vector3 box2halfsize = m_lBox[j]->n_Size / 2.0f;
			vector3 trans2 = vector3(m_lMatrix[j][3]);
			vector3 box2center = trans2 + m_lBox[j]->n_Center;

			vector3 dist = glm::abs(box1center - box2center);

			vector3 sumHalfSizes = box1HalfSize + box2halfsize;
			...
			if(bColliding)
			{
				...
				m_lBox[i]->n_Color = m_lBox[j]->n_Color = MERED; //We make the outside box of both models red
			}
		}
	}
}